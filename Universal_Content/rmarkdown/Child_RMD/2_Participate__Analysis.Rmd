...
```{r, part-stdnts, cache=TRUE, include=FALSE}
	indiv_smry <- Report_Data[[assessment]][VALID_CASE=='VALID_CASE' & YEAR %in% c(2019, 2021) & GRADE %in% params$sgp.grades$State_Assessment, list(
	#Report_Data[[assessment]][VALID_CASE=='VALID_CASE' & eval(parse(text = params$grd.subset)), list(
                  ...
                  Score_Corr_Baseline = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_2YEAR, use="pairwise.complete"), 2),
									Total_Students = .N,
									Baseline_Count = sum(!is.na(eval(parse(text = params$baseline.sgp))))), keyby = list(CONTENT_AREA, GRADE, YEAR)]
	indiv_smry[, Pct_Baseline := round((Baseline_Count/Total_Students)*100, 1)]

	indiv_counts <- Report_Data[[assessment]][VALID_CASE=='VALID_CASE' & YEAR %in% c(2019, 2021) & GRADE %in% params$sgp.grades$State_Assessment, list(
	                             # eval(parse(text = params$grd.subset)), list(
									Total_Students = .N,
	...
	baseline.pct <- round((indiv_counts$Baseline_Count/indiv_counts$Total_Students)*100, 1)
	...
	# Report_Data[[assessment]][, SCALE_SCORE_STANDARDIZED := scale(SCALE_SCORE), by=c("CONTENT_AREA", "GRADE")]
	# missing <- Report_Data[[assessment]][is.na(eval(parse(text = params$cohort.sgp))) & !is.na(eval(parse(text = params$baseline.sgp))) & eval(parse(text = params$grd.subset)) & CONTENT_AREA %in% params$subject_order,
	# 												 list(SCALE_SCORE_STANDARDIZED, SCALE_SCORE_PRIOR_2YEAR, eval(parse(text = params$baseline.sgp)), CONTENT_AREA, GRADE)]
	# setnames(missing, "V3", params$baseline.sgp)
	# missing_smry <- missing[, list(
	# 												Mean_SS = mean(SCALE_SCORE, na.rm = TRUE),
	# 												Mean_SS_Prior = mean(SCALE_SCORE_PRIOR_2YEAR, na.rm = TRUE),
	# 												Mean_SGP = mean(eval(parse(text = params$baseline.sgp)), na.rm = TRUE),
	# 												N = .N)]#, keyby = c("CONTENT_AREA", "GRADE")]

	# indiv.q80 <- as.numeric(Report_Data[[assessment]][VALID_CASE=='VALID_CASE' & eval(parse(text = params$grd.subset)),
	# 														as.list(quantile(abs(eval(parse(text = params$cohort.sgp)) - eval(parse(text = params$baseline.sgp))), probs=0.8, na.rm=TRUE))])
```

```{r, part-counts-raw, results='asis'}
	###  Create SGP_Counts object for table in "Counts" section
	SGP_Counts <- indiv_smry[GRADE %in% params$sgp.grades$State_Assessment, c(1:3, 7:9)] # Percentages receiving SGPs
	if (any(as.numeric(params$sgp.grades$State_Assessment)>9)) {
	...
	setnames(SGP_Counts, c("Content Area", "Grade", "Year", "Total Students", "Count", "Percent"))
...
###  Conditional text for Participation section.
  ...
  # if (is.null(params$skip.grades.list)) {
	# 	skip.excluded.grade <- paste0(ord(as.numeric(setdiff(params$sgp.grades$State_Assessment, params$skip.grades))), " grade")
	# } else { #  4<sup>th</sup> grade ELA and mathematics and 5<sup>th</sup> grade science
	# 	low.grades <- sort(sapply(sapply(params$sgp.grades$State_Assessment.list, as.numeric), min))
