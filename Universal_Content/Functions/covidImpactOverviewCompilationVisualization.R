##############################################################################
###
### covidImpactOverviewCompilationVisualization function
###
##############################################################################
...
overall_file_name <- ifelse(academic_impact_metric=="Hybrid", paste("Academic_Impact_Overview_Overall_COMPILATION_Hybrid", academic_impact_groups_label, sep="_"), paste("Academic_Impact_Overview_Overall_COMPILATION", academic_impact_groups_label, sep="_"))
quintile_1_file_name <- ifelse(academic_impact_metric=="Hybrid", paste("Academic_Impact_Overview_Quintile_1_COMPILATION_Hybrid", academic_impact_groups_label, sep="_"), paste("Academic_Impact_Overview_Quintile_1_COMPILATION", academic_impact_groups_label, sep="_"))
quintile_2_file_name <- ifelse(academic_impact_metric=="Hybrid", paste("Academic_Impact_Overview_Quintile_2_COMPILATION_Hybrid", academic_impact_groups_label, sep="_"), paste("Academic_Impact_Overview_Quintile_2_COMPILATION", academic_impact_groups_label, sep="_"))
...
if (anonymize_overview_groups) {
    tmp_length <- length(Academic_Impact_Overview_COMPILATION_PROCESSED)
    names(Academic_Impact_Overview_COMPILATION_PROCESSED) <- paste("State", sample(LETTERS[seq(tmp_length)], tmp_length, replace=FALSE))
...
if ("OVERALL" %in% visualization_type) {
    ...
    pushViewport(academic_impact_overview_vp)
        ...
        grid.text(x=title_x_coors, y=length(Academic_Impact_Overview_COMPILATION_PROCESSED)+0.8, sapply(names(parameters[['grades']]), capwords), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], cex=2.1), just="center", default.units="native")
        grid.text(x=x_coors_dt[['X_COOR']], y=length(Academic_Impact_Overview_COMPILATION_PROCESSED)+0.25, paste("Grade", unlist(parameters[['grades']])), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], cex=1.4), just="center", default.units="native")
        row_y_coors <- seq_along(names(Academic_Impact_Overview_COMPILATION_PROCESSED)) - rep(0.55, length(names(Academic_Impact_Overview_COMPILATION_PROCESSED)))
        grid.text(x=2.2, y=row_y_coors, as.character(sapply(rev(names(Academic_Impact_Overview_COMPILATION_PROCESSED)[name_order]), capwords)), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], cex=rep(1.3, length(Academic_Impact_Overview_COMPILATION_PROCESSED))), just="right", default.units="native")
...
if ("QUINTILE_1" %in% visualization_type) {
    ...
    pushViewport(title_vp)
        ...
        grid.text(x=0.01, y=0.5, paste("Multi-State", strtail(parameters[['prior_year']], 4), "to", strtail(parameters[['current_year']], 4), "COVID-19 Academic Impact"), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], fontface=2, fontfamily="Helvetica-Narrow", cex=2.9), just="left", default.units="native")
        grid.text(x=0.99, y=0.7, capwords(academic_impact_groups), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], fontface=2, fontfamily="Helvetica-Narrow", cex=2.0), just="right", default.units="native")
        grid.text(x=0.99, y=0.3, paste(paste(sapply(content_area_label, capwords), collapse=" and "),  "by Grade and Achievement Quintile"), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], fontface=2, fontfamily="Helvetica-Narrow", cex=2.0), just="right", default.units="native")
    popViewport() ## title_vp
    ...
    pushViewport(academic_impact_overview_vp)
        ...
        row_y_coors <- seq_along(names(Academic_Impact_Overview_COMPILATION_PROCESSED)) - rep(0.5, length(names(Academic_Impact_Overview_COMPILATION_PROCESSED)))
        grid.text(x=2.2, y=row_y_coors, as.character(sapply(rev(names(Academic_Impact_Overview_COMPILATION_PROCESSED)[name_order]), capwords)), gp=gpar(col=parameters[['graphic_format']][['colors_font']][1], cex=rep(1.3, length(Academic_Impact_Overview_COMPILATION_PROCESSED))), just="right", default.units="native")
        if (length(parameters[['grades']]) > 1) {
            x_coors_content_area_transitions <- head(cumsum(sapply(parameters[['grades']], length)), -1)
            for (x_coors_content_area_transitions_iter in x_coors_content_area_transitions) {
...
} ### END covidImpactOverviewCompilationVisualization function
